...
const typeColors: Record<string, string> = {
  "pre-wedding": "bg-blue-500/10 text-blue-600 dark:text-blue-400",
  "bachelor-party": "bg-amber-500/10 text-amber-600 dark:text-amber-400",
  "wedding-week": "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400",
...
export default function EventCard({ event, variant = "full" }: Props) {
  if (variant === "compact") {
    return (
      <Link
        ...
        className="flex items-center gap-4 rounded-xl border bg-card p-4 transition-colors hover:border-gold/50"
      >
  ...
  return (
    <Link
      to={`/events/${event.slug}`}
      className="group block rounded-xl border bg-card p-5 transition-all hover:border-gold/50 hover:shadow-lg"
    >
      ...
      <h3 className="mb-3 font-display text-xl font-semibold group-hover:text-gold transition-colors">
        {event.name}
      </h3>
      ...
      <div className="flex items-center gap-2 rounded-lg bg-gold-muted px-3 py-2 text-sm">
